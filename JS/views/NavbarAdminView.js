...
function createNavBar() {
  ...
  if (User .isLogged()) {
    console.log('User  is logged in ' + User.getUserLogged().name); // Corrigido aqui
    // Cria o link do user e o botão de logout
    const userLink = document.createElement('a');
    userLink.className = 'text-gray-900 hover:text-orange-500';
    userLink.textContent = User.getUserLogged().name; // Corrigido aqui
...
// Função para renderizar a barra de navegação na div navbarAdmin
function renderNavBar() {
  const navbarContainer = document.getElementById('navbarAdmin'); // Corrigido para navbarAdmin
  const navBar = createNavBar();
  navbarContainer.appendChild(navBar);
}

// Chama a função para renderizar a barra de navegação quando o DOM estiver carregado
